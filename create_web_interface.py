...
# -*- coding: utf-8 -*-
"""
Создание простого веб-интерфейса для Shannon-Uncontained
"""
...
def create_web_interface(client):
    """Создать простой веб-интерфейс"""
    print("\n" + "=" * 60)
    print("Создание веб-интерфейса")
    print("=" * 60)
    
    web_interface_code = """#!/usr/bin/env node
/**
...
<body>
    ...
    // Создать веб-сервер
    server_code = f"""const http = require('http');
const fs = require('fs');
const path = require('path');
const {{ exec }} = require('child_process');
const {{ promisify }} = require('util');
...
"""
    ...
    # Сохранить файл
    server_file = f"{PROJECT_PATH}/web-interface.js"
    
    command = f"""cat > {server_file} << 'SERVEREOF'
{server_code}
SERVEREOF
chmod +x {server_file}
"""
    ...
    if exit_status == 0:
        print(f"[OK] Веб-интерфейс создан: {server_file}")
        print("\n[INFO] Для запуска:")
        print(f"  node {server_file}")
        print("\n[INFO] Доступ:")
        print(f"  http://72.56.79.153:3000")
        print("\n[INFO] Для запуска в фоне:")
        print(f"  nohup node {server_file} > web-interface.log 2>&1 &")
    else:
        print(f"[WARN] Ошибка создания интерфейса: {error[:200]}")

def main():
    print("=" * 60)
    print("Создание веб-интерфейса для Shannon-Uncontained")
    print("=" * 60)
    ...
    try:
        ...
        create_web_interface(client)
        
        print("\n" + "=" * 60)
        print("[OK] Веб-интерфейс создан!")
        print("=" * 60)
